...
class ProjectGrid extends HTMLElement {
    connectedCallback() {
        ...
        grid.innerHTML = SITE_DATA.projects.map(p => {
            ...
            const gradientColor = p.color === 'yellow' ? 'from-orange-500 to-yellow-500' : 'from-gray-500 to-white';
            const iconColor = p.active ? 'text-yellow-500 group-hover:text-yellow-400' : 'text-tech-dim';
            const borderColor = p.active ? 'group-hover:border-yellow-500/50' : '';
            const iconName = p.active ? 'sun' : 'box'; 
            ...
            <article class="${baseClass} ${activeClass} min-h-[300px]">
                ${p.active ? `<div class="absolute top-0 left-0 w-full h-1 bg-gradient-to-r ${gradientColor} opacity-70"></div>` : ''}
                
                <div class="p-6 flex flex-col h-full">
                    <div class="flex justify-between items-start mb-4">
                        <div class="w-12 h-12 bg-tech-gray/30 rounded flex items-center justify-center ${iconColor} border border-tech-gray ${borderColor} transition-colors">
